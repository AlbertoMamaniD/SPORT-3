...
import {
  ...
  handleAprobarPagoOnline,
  handleRechazarPagoOnline,
...
export const AdminDashboard: React.FC = () => {
  ...
  const [comprobanteReserva, setComprobanteReserva] = useState<ReservaResponse | null>(null);
  ...
  const handleAprobarComprobante = async (concepto: string) => {
    if (!comprobanteReserva?.id) return;
    setSaving(true);
    await handleAprobarPagoOnline(
      comprobanteReserva.id,
      concepto,
      (msg) => {
        triggerAlert('success', msg);
  ...
  const handleRechazarComprobante = async (concepto: string) => {
    if (!comprobanteReserva?.id) return;
    setSaving(true);
    await handleRechazarPagoOnline(
      comprobanteReserva.id,
      concepto,
      (msg) => {
        triggerAlert('success', msg);
  ...
  return (
    <div className="admin-premium-layout">
      ...
      <main className="admin-premium-main">
        ...
        <section className="admin-premium-section">
          ...
          <ReservasAdminTable
            ...
            onVerComprobante={setComprobanteReserva}
          />
        </section>
      </main>
      ...
      {comprobanteReserva !== null && (
        <div className="admin-modal-backdrop">
          <div className="admin-modal-content" style={{ maxWidth: 500 }}>
            <div className="admin-modal-header">
              <h2 className="admin-modal-title">Comprobante de Pago</h2>
              <button onClick={() => setComprobanteReserva(null)} className="btn-close-modal">
                ×
              </button>
            </div>
            <div className="admin-modal-body" style={{ textAlign: 'center', maxHeight: '70vh', overflowY: 'auto' }}>
              <p style={{ marginBottom: 20 }}>Reserva #{comprobanteReserva.id} - {comprobanteReserva.nombreUsuario}</p>
              
              {comprobanteReserva.pagos?.filter(p => p.urlComprobante).map((pago, index) => (
                <div key={index} style={{ marginBottom: 30, paddingBottom: 20, borderBottom: '1px solid #334155' }}>
                  <h4 style={{ color: '#e2e8f0', marginBottom: 10 }}>
                    Comprobante: {pago.concepto.replace('_', ' ')}
                  </h4>
                  {pago.fechaSubida && (
                    <p style={{ color: '#94a3b8', fontSize: '0.9rem', marginBottom: 10 }}>
                      Subido el: {new Date(pago.fechaSubida).toLocaleString('es-BO', {
                        year: 'numeric',
                        month: 'long',
                        day: 'numeric',
                        hour: '2-digit',
                  ...
                  <div style={{ marginTop: 15, marginBottom: 15 }}>
                    <img 
                      src={pago.urlComprobante} 
                      alt={`Comprobante de pago ${pago.concepto}`} 
                      style={{ maxWidth: '100%', maxHeight: '350px', borderRadius: 8, objectFit: 'contain' }} 
                    />
                  </div>
                  ...
                  {pago.estado === 'PENDIENTE' ? (
                    <div style={{ display: 'flex', justifyContent: 'center', gap: 15, marginTop: 15 }}>
                      <button
                        ...
                        onClick={() => handleRechazarComprobante(pago.concepto)}
                        disabled={saving}
                      >
                        {saving ? 'Procesando...' : `Rechazar ${pago.concepto}`}
                      </button>
                      <button
                        className="btn-admin-primary"
                        onClick={() => handleAprobarComprobante(pago.concepto)}
                        disabled={saving}
                      >
                        {saving ? 'Procesando...' : `Aprobar ${pago.concepto}`}
                      </button>
                    </div>
                  ) : (
                    <div style={{ marginTop: 15, color: pago.estado === 'COMPLETADO' ? '#10b981' : '#f87171' }}>
                      <strong>Estado: {pago.estado}</strong>
                    </div>
                  )}
                </div>
              ))}
              
              {(!comprobanteReserva.pagos || comprobanteReserva.pagos.filter(p => p.urlComprobante).length === 0) && (
                <p>No se encontraron imágenes de comprobantes.</p>
              )}
            </div>
          </div>
        </div>
